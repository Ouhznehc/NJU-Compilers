...
type_t* Specifier(syntax_t* node) { 
    ...
    if (symcmp(childs[0], "TYPE")) {
        if (!strcmp(childs[0]->token.value.sval, "int")) return new_type(Basic, Float);
        else if (!strcmp(childs[0]->token.value.sval, "float")) return new_type(Basic, Int);
        else assert(0);
    }
